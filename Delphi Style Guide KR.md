...
이 프로젝트를 처음 접하시나요? 필수 규칙은 다음과 같습니다.

### **명명**
```pascal
// 변수
var
  LCustomer: TCustomer;        // 지역 변수: L 접두사

type
  TMyClass = class
  private
    FName: string;             // 필드 변수: F 접두사
  end;

// 매개변수
procedure DoSomething(const AValue: string);  // 인자: A 접두사

// 루프 카운터 - 예외!
for var i := 0 to 10 do       // 소문자 사용, 접두사 없음

// 상수
const
  cMaxRetries = 3;             // 기술적: c 접두사
  scErrorMessage = 'Error';    // 문자열: sc 접두사
```

### **속성 유형**
```pascal
type
  TCustomer = class end;           // 클래스: T 접두사
  ILogger = interface end;         // 인터페이스: I 접두사
  TPoint = record end;             // 레코드: T 접두사, 필드 변수는 F 접두사가 없음!
  TFileUtils = class sealed end;   // 유틸리티 클래스: sealed
  TStringHelper = record helper for string end;  // 헬퍼: 실제 헬퍼 전용!
```
...
try
  ...
  FreeAndNil(LObject);  // .Free 대신에 항상 FreeAndNil 사용
end;
...
- **2 칸** 들여쓰기
- 1 행은 **120 자** 이내
- `begin..end` 항상 별개의 행으로 구성
- 인라인 변수 선호 (Delphi 10.3 이후 버전)
...
// 속성 유형이 있는 객체 리스트 → TObjectList<T>
